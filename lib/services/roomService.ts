...
export class RoomService {
  ...
  private static fetchFromDbCached = unstable_cache(
    async () => {
      console.log(
        "[RoomService] Cache miss (Redis & Next.js). Fetching DB for rooms.",
      );
  ...
  static getRooms = cache(async () => {
    ...
    try {
      ...
      if (rooms && rooms.length > 0) {
        ...
        if (redis) {
          redis
            .set(this.CACHE_KEY, JSON.stringify(rooms), { ex: this.CACHE_TTL })
            .catch(() => {});
        }
        return rooms;
      }
  ...
  static async updateRoom(id: string, data: any) {
    ...
    if (current?.images && Array.isArray(current.images)) {
      const currentKeys = current.images
        .map((img: any) => img.key)
        .filter(Boolean);
      const newKeys = Array.isArray(data.images)
        ? data.images.map((img: any) => img.key).filter(Boolean)
        : [];
      ...
      if (keysToDelete.length > 0) {
        try {
          ...
          console.log(
            `[RoomService] Old gallery images deleted:`,
            keysToDelete,
          );
        } catch (error) {
          console.error(
            `[RoomService] Failed to delete old gallery images:`,
            error,
          );
        }
      }
